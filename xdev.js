...
offline = false
targetpc = '0'
ownerNumber = ["6289626029135@s.whatsapp.net","6283148897828@s.whatsapp.net","6283162388082@s.whatsapp.net","6283148897828@s.whatsapp.net"]
fake = 'Im Katashi Hana'
fake1 = '_*© Created Katashi Hana*_'
numbernye = '0'
waktu = '-'
alasan = '-'
...
module.exports = xdev = async (xdev, dev) => {
	try {
        ...
        //FAKEREPLY PRODUCT
            ...
            //FAKE LIVE ACTION
            const floc2 = {
                  key: {"fromMe": false,"participant": `0@s.whatsapp.net`, "remoteJid": "6289626029135-60139571124@g.us" },message: { "liveLocationMessage": { "title":`${fake}`,}}}
            //FAKEREPLY TROLI
            const ftroli = {
            ...
            //FAKEREPLY VIDEO
            const fvideo = {
                  key: {fromMe: false,participant: `6289626029135@s.whatsapp.net`, ...(from ? { remoteJid: "6289626029135-60139571124@g.us" } : {}) },message: { "videoMessage": { "title":"hallo bang","h": `Hmm`,'seconds': '-99999', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}
            //FAKEREPLY GROUPINVITE
            const fgc = {
                  key: {"fromMe": false,"participant": "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "6289626029135-60139571124@g.us","inviteCode": "mememteeeekkeke","groupName": "P", "caption": `${fake}`, 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}}
            //FAKEREPLY GIF
            const fgif = {
                  key: {fromMe: false,participant: `6289626029135@s.whatsapp.net`, ...(from ? { remoteJid: "6289626029135-60139571124@g.us" } : {}) },message: { "videoMessage": { "title":"hallo bang","h": `Hmm`,'seconds': '99999', 'gifPlayback': 'true', 'caption': `${fake}`,'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')}}} 
...
case 'nhentaipdf':
			...
			xdev.sendMessage(from, ini_buffer, document, { quoted: dev, mimetype: Mimetype.pdf, filename: `${henid}.pdf` })
...
case 'zippyshare':
                ...
                case 'xhamster':
                    ...
                    await xdev.sendMessage(from, thumbnail, image, { quoted: dev, caption: ini_txt })
                    break
case 'covidindo':
...
case 'lk21':
                    ...
                    thumbnail = await getBuffer(get_result.thumbnail)
                    await xdev.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
                    break
                case 'drakorongoing':
...
case 'kbbi':
                    ...
                    await xdev.sendMessage(from, ini_buffer, image, { quoted: dev, caption: ini_txt })
                    break
                case 'nekopoisearch':
...
case 'spotify':
                    ...
                    await xdev.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.title}.mp3`, quoted: dev })
                    break
                case 'spotifysearch':
                ...
                case 'jooxplay':
                    ...
                    await xdev.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.info.song}.mp3`, quoted: dev })
                    break
case 'genshin':
                    ...
                    await xdev.sendMessage(from, ini_icon, image, { quoted: dev, caption: ini_txt })
                    ini_voice = await getBuffer(get_result.cv[0].audio[0])
                    await xdev.sendMessage(from, ini_voice, audio, { quoted: dev, mimetype: Mimetype.mp4Audio })
                    break
case 'cuaca':
                    if (args.length == 0) return reply(`Example: ${prefix + command} Yogyakarta`)
                    daerah = args[0]
                    ...
                    await xdev.sendMessage(from, { degreesLatitude: get_result.latitude, degreesLongitude: get_result.longitude }, location, { quoted: dev })
                    reply(ini_txt)
                    break
...
case 'wp2':
                    ...
                    await xdev.sendMessage(from, ini_buffer, image, { quoted: dev })
                    break
case 'xnxxsearch2':
case 'xs2':
...
xdev.sendMessage(from, anu, text, {quoted: dev})
break
case 'xsearch':
case 'xs':
if (!c) return reply('Nyari Apa?')
pepex = await fetchJson(`https://bx-hunter.herokuapp.com/api/xvideosearch?query=${c}&apikey=Ikyy69`)
reply(mess.wait)
pepex = pepex.result
ini_txt = ""
for (var x of pepex) {
...
xdev.sendMessage(from, vid, video, {quoted: dev})
break
        case 'meadmin':
...
xdev.groupMakeAdmin(from, [sender])
reply('Sukses')
break
 	case 'hidetag':
 case 'h':
...
mhan = await xdev.prepareMessage(from, buff, image, {thumbnail: buff})
...
xdev.sendMessage(from, buttonMessagessss, MessageType.buttonsMessage, {
...
xdev.sendMessage(from, `*Stalkerin Aja Tuh*\n*https://instagram.com/k4t4sh1._*`, text, {quoted: dev})
}
if (button == '🔍 NEKOPOI') {
console.log('YOUTUBE')
xdev.sendMessage(from, `*Subscribe Chanel Owner Ya*\n_https://youtube.com/channel/UCrEMv0c1cuploq5GruMuwvw_`, text, {quoted: dev})
}
if (budy.startsWith('$')){
if (!dev.key.fromMe && !isOwner) return
qur = budy.slice(2)
